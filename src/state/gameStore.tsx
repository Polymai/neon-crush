...
import type { Board, BoosterKey, GameStatus, LevelDefinition, LevelResult, Position, TileKind } from "../game/types";
...
function createInitialState(levelId = 1): GameState {
  ...
  return {
    ...
    goalsCleared: {},
    status: "playing",
    message: "Swap adjacent tiles to start a chain.",
    startedAt: Date.now(),
    lastResult: null,
  };
}
...
export function GameProvider({ children }: { children: ReactNode }) {
  ...
  const resolveSwap = useCallback((current: GameState, a: Position, b: Position): GameState => {
    ...
    if (!detectMatches(swapped).length) {
      neonAudio.blip("swap");
      return { ...current, selected: null, message: "No match there. Try another neon line." };
    }
  ...
  const selectTile = useCallback(
    (position: Position) => {
      setState((current) => {
        ...
        if (!current.selected) return { ...current, selected: position, message: "Choose an adjacent tile to swap." };
        if (isSamePosition(current.selected, position)) return { ...current, selected: null, message: "Selection cleared." };
        if (!isAdjacentPosition(current.selected, position)) return { ...current, selected: position, message: "Selection moved." };
